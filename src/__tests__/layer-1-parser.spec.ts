...
describe('should handle partial parses', () => {
  const looseBadStrings: [string, IToken[]][] = [
    // [
    //   '11',
    //   [NT(11), LET]
    // ],
    [
      '11.1.1.',
      [NT(11), PT, NT(1), PT, NT(1), PT, LET]
    ]
  ]
  const badStrings: [string, TokenCollection][] = looseBadStrings.map((row: [string, IToken[]]) => [row[0], new TokenCollection(row[1])])
  badStrings.forEach((pattern) => {
    const [rawString, expectedTokens]: [string, TokenCollection] = pattern
    it(`${rawString} -> ${expectedTokens}`, () => {
      const tokenised = new Layer0Parser().parse(rawString).processLayer1()
      expect(tokenised.length).to.eq(expectedTokens.length)
